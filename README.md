...
- Las migraciones de EF Core corren automáticamente al iniciar el contenedor de la API (`dbContext.Database.MigrateAsync()` en `Program.cs`, solo fuera de `Development`, **antes** de `AdminUserSeeder.Seed()`) — no hace falta correr `dotnet ef database update` a mano en Test/Prod. El orden importa: es `Migrate()` quien crea la base la primera vez, así que tiene que correr antes de que el seeder intente consultarla (ver el healthcheck de `sqlserver` más abajo, que evita además que la API arranque antes de que el motor esté listo).
...
- `sqlserver` tiene un `healthcheck` (`sqlcmd -Q 'SELECT 1'`) y `api-test`/`api-prod` usan `depends_on: sqlserver: condition: service_healthy` — sin esto, la API arranca en cuanto el puerto TCP de SQL Server responde, que sucede antes de que el motor esté listo para autenticar conexiones, y el contenedor queda en loop de restart en un arranque en frío (Error 4060, "Cannot open database").

...
**Nota — proceso actual de consentimiento (relevado 2026-08-05):** el responsable describe un mecanismo ya en uso hoy — nombre completo + huella + registro de IP/ubicación/fecha-hora — pero desconoce el detalle técnico de su implementación interna. Esto no es equivalente a una firma electrónica certificada por un proveedor (DocuSign/Dropbox Sign), y no está claro si cumple de forma robusta con el ESIGN Act. Verificado en el código de este repo: el único rastro relacionado es `Policy.ConsentSigned` (`bool?`, checkbox simple, sin lógica de IP/ubicación/huella asociada — ver §2) — el mecanismo descrito por el responsable no parece estar implementado en este CRM, lo que sugiere que es externo o manual, pero falta confirmarlo.

- **(a) Sigue abierto, no resuelto todavía:** si este mecanismo (huella + IP/ubicación/fecha-hora) está implementado dentro del código actual del CRM o es un proceso externo/manual. No confirmado por el responsable todavía.
- **(b) Resuelto (2026-08-05):** el responsable confirmó que **Dropbox Sign debe reemplazar totalmente** este proceso — no van a convivir. **Condición:** el reemplazo queda sujeto a validar que Dropbox Sign cumpla con la normativa aplicable (ESIGN Act y cualquier requisito estatal de seguros vigente) antes de dar de baja el proceso actual de huella/IP. No es un reemplazo incondicional desde el arranque de la integración técnica — el mecanismo actual se mantiene activo hasta confirmar ese compliance.

Mientras no se aclare (a), no asumir que el reemplazo es operacionalmente limpio — podría haber lógica o datos existentes atados al mecanismo actual (huella/IP) que haya que migrar o conciliar antes de darlo de baja.

**Punto nuevo, no documentado hasta ahora (relevado 2026-08-07):** aparte del mecanismo de huella/IP de arriba, el responsable aclaró que existe un **sistema separado, legado, fuera de este CRM**, usado para el acuerdo de agente (contrato de marketing/agente) — un documento de ejemplo relevado en conversación anterior mostraba un audit trail de un proveedor de e-signature real (no huella/IP). Esto **no está resuelto por ya tener un proveedor funcionando ahí**: es un proceso paralelo que también hay que resolver/reemplazar como parte de este mismo trabajo, integrándolo al CRM. **Pregunta abierta para el responsable, no asumir la respuesta:** ¿ese acuerdo de agente se va a firmar a través de la misma integración de Dropbox Sign que se está construyendo para consentimiento de pólizas de clientes, o es un flujo aparte con sus propias reglas (proveedor, formato, guardado)?
...
**✅ Resuelto (2026-07-27, §15.3; caso puntual cerrado, verificado 2026-08-07)**: las 2490 filas migradas que habían quedado con `Customer.AgentId` en el fallback (primer User con Rol=Admin) se reasignaron una vez cargados los 41 agentes reales como `User` (§15.2), matcheando por nombre contra el reporte JSON original (campo `AgentFallbacks`) — 1178/1179 pólizas reasignadas (99.92%). El caso puntual restante (póliza 381835 de Mariana Salvador Cruz) ya no está en fallback — ver detalle y alcance de la verificación en §23.2.
...
**Hallazgo aparte, visto en el camino — ✅ Cerrado (verificado 2026-08-07)**: la póliza 381835 de Mariana (la que traía "Salvador Cruz" sin SSN real) había tenido `AgentId = 1` (el Admin seedeado, fallback de `EntityMatcher.ResolveAgentAsync` — ver §7/§15.3) en vez de un agente real, porque el nombre de agente de esa fila de origen no matcheó ningún `User` existente al momento de migrar.

**Estado actual (verificado 2026-08-07, consultado directo en base):** `Customer.AgentId` de esa póliza ya **no** está en fallback — apunta a `User.Id = 3046` ("YOJANA HERRERA", `Rol = Agente`, uno de los 41 agentes reales de §15.2). No se investigó cuándo ni cómo se corrigió — no hay registro de que se haya hecho en una sesión de trabajo documentada, y no cambia la acción a tomar; se documenta el estado tal cual se encontró.

**Alcance de esta verificación:** se confirmó **únicamente contra la base de desarrollo local** (`(localdb)\MSSQLLocalDB`, `WholeCareInsuranceDb`), la única base que existe hoy. Los ambientes de Test/Prod (`WholeCareInsuranceDb_Test`/`WholeCareInsuranceDb_Prod`, ver §8.1) **todavía no existen** — el despliegue real al VPS sigue pendiente — por lo que no hay nada que verificar ahí todavía. Cuando se despliegue Test/Prod, este dato deberá confirmarse de nuevo en esas bases (probablemente sembradas de forma independiente).
...
46. ~~Customers duplicados por bug de matching en la migración (Doris Maldonado, Mariana Salvador Cruz)~~ ✅ Hecho: los 2 casos confirmados fusionados con backup previo y verificación (§23.2); refuerzo del matching (`_customerByDob` + `CheckPossibleDuplicate`, solo reporta, nunca fusiona automáticamente) implementado y verificado (§23.3). El `AgentId` en fallback Admin encontrado en la póliza de Mariana ya no está en fallback (verificado 2026-08-07 contra la base de desarrollo local — ver §23.2 para alcance y detalle; Test/Prod no verificados, no existen todavía)

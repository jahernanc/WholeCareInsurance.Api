...
## 24. Rediseño de la relación Customer ↔ Customer (titular/dependiente-aplicante) — ✅ Hecho (2026-07-30): las 4 fases (schema, backfill de 884 filas, endpoints backend, frontend) implementadas y verificadas end-to-end. Quedan 3 puntos operativos sin bloquear nada — ver §24.9
...
2. **69 dependientes con email real (no placeholder) que quedaron fuera del alcance de la Fase 2** — detectados al responder la pregunta de por qué `1181 + 879 ≠ 2128` (ver conversación de esta sesión, 2026-07-30): tienen `PolicyDependent` real pero nunca se les generó `CustomerRelationship` porque la Fase 2 se scopeó específicamente a los 947 Customers con email placeholder, no a todos los dependientes. No es un bug — la Fase 2 hizo exactamente lo que se le pidió — pero es un hueco de cobertura real. Candidato a una mini-fase aparte: mismo INSERT de §24.6 pero sin el filtro `Email LIKE 'noemail+P%'`, con su propio dry-run/backup. El Customer `Id 9` ("Carlos Mendez", `carlos.mendez@example.com`) es el único caso sin ningún vínculo (ni `Policy`, ni `PolicyDependent`, ni `CustomerRelationship`) — parece dato de prueba/seed anterior a la migración real, no un Customer de negocio.
...
47. ~~Rediseño de la relación Customer ↔ Customer (titular/dependiente-aplicante)~~ ✅ Hecho (§24): Fase 1 (modelo `CustomerRelationship`, migración) + Fase 2 (backfill de 884 filas, §24.6) + Fase 3 (endpoints backend, filtro `?role=`, KPI `UniqueMembersCount`, §24.7) + Fase 4 (filtro de rol en Customers.jsx, tarjeta "Personas únicas" en el Dashboard, §24.8) — verificado end-to-end con SQL directo, curl y navegador. Quedan 3 puntos operativos sin bloquear nada (§24.9): revisión manual de Clara/Elizabeth, mini-fase de los 69 dependientes de email real sin migrar, backfill manual de emails placeholder.
